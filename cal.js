...
function updateClientPortion() {
  ...
  client_portion = (cash_investment - aum).toFixed(2) || 0;
  ...
  client_portion_percentage =
    1 - ((financing_ratio / 100) * d1_sv) / d1_total_premium || 0;
...
function updateTotalPortionDetail() {
  ...
  for (let i = 0; i < detail_planInput.length; i++) {
    ...
    console.log(parseFloat(total_value_detail[i]) + "+" + multi);
...
plan.addEventListener("change", function () {
  ...
  var url = new URL(config.apiUrl + "/getTotalPortion");
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      console.log(total_value_detail);
      for (let i = 0; i < 31; i++) {
