...
function updateClientPortion() {
  ...
  discount_amount = (policy_discount_rate * total_portion).toFixed(0);
...
function updateNetReturn() {
  ...
  for (let i = 0; i < net_returnInput.length; i++) {
    ...
    let net_return_year = (annual_int_return - annual_int_paid).toFixed(0);
    net_return = parseFloat(net_return);
    net_return += parseFloat(net_return_year);
    //console.log(i + ":" + net_return);
    net_returnInput[i].value = parseFloat(
      net_return.toFixed(0)
    ).toLocaleString();
